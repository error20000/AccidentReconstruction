...
var TimeLineEventData = [
	{"name":"事故背景介绍","timeStart":0,"index":[],"timeLength":Utils.timeLength("00:01", "01:15"),"time":["",""],select:true,event: function(timePoint, timeEvent){
		...
		if(timePoint == timeEvent.timeStart + 45){
			Utils.addDescMsg(3, "<b>2018年1月5日0106时许</b>，该轮抵达韩国釜山港外锚地，在锚地完成加油（加重油1050吨、轻油90吨和润滑油17.43吨）后于当日1430时起锚续航，开航时航行灯开启，主机转速为87RPM。");
		}
		if(timePoint == timeEvent.timeStart + 66){
			Utils.addDescMsg(4, "<b>1月6日1200时许</b>，船长指令将主机转速加到100RPM（实际97 RPM），此后该轮定速航行。");
		}
	}},
	{"name":"开始靠近（19001号交接班）","timeStart":0,"index":[],"timeLength":Utils.timeLength("01:16", "01:52"),"time":["2018/1/6 19:00:00","2018/1/6 19:30:00"],select:true,event: function(timePoint, timeEvent){
		//time
	...
	{"name":"继续靠近（1930-1939）","timeStart":0,"index":[],"timeLength":Utils.timeLength("01:53", "04:03"),"time":["2018/1/6 19:30:00","2018/1/6 19:39:00"],select:true,event: function(timePoint, timeEvent){
		//time
		var startTime = timeEvent.time[0];
		var endTime = timeEvent.time[1];
		var interval = (Math.floor(new Date(endTime).getTime()/1000) - Math.floor(new Date(startTime).getTime()/1000))/timeEvent.timeLength; //单位秒
		...
		}else if(timePoint == Math.floor(timeEvent.timeStart + 2*60/interval)){ //"2018/1/6 19:34:00"
		...
		}else if(timePoint == Math.floor(timeEvent.timeStart + 2*60/interval+3)){ //"2018/1/6 19:34:00"
		...
		}else if(timePoint == Math.floor(timeEvent.timeStart + 3*60/interval)){ //"2018/1/6 19:35:00"
		...
		}else if(timePoint == Math.floor(timeEvent.timeStart + 4*60/interval)){ //"2018/1/6 19:36:00"
		...
		}else if(timePoint == Math.floor(timeEvent.timeStart + 5*60/interval)){ //"2018/1/6 19:39:00"
	...
	{"name":"碰撞前10分钟（1940-1949）","timeStart":0,"index":[],"timeLength":Utils.timeLength("04:04", "10:50"),"time":["2018/1/6 19:41:00","2018/1/6 19:49:00"],select:true,event: function(timePoint, timeEvent){
		//time
		var startTime = timeEvent.time[0];
		var endTime = timeEvent.time[1];
		var interval = (Math.floor(new Date(endTime).getTime()/1000) - Math.floor(new Date(startTime).getTime()/1000))/timeEvent.timeLength; //单位秒
...
//			Utils.addDescMsg(22, "<b>1947时许</b> 2号值班水手提醒三副，在雷达上观察到“1”轮AIS物标，CPA约0.2海里，三副也注意到该物标并判断来船为小船。");
		...
		if(timePoint == timeEvent.timeStart + 350){ //"2018/1/6 19:49:00"
		...
		if(timePoint == timeEvent.timeStart + 350 + 1){ //"2018/1/6 19:49:00"
			Utils.appendShipInfo(0, {
				ro: '左满舵'
			});
		}
		if(timePoint == timeEvent.timeStart + 350 + 2){ //"2018/1/6 19:49:00"
			Utils.appendShipInfo(0, {
				ro: '右满舵'
			});
		}
		if(timePoint == timeEvent.timeStart + 350 + 3){ //"2018/1/6 19:49:00"
			Utils.appendShipInfo(0, {
				ro: '右满舵'
			});
		}
		if(timePoint == timeEvent.timeStart + 350 + 4){ //"2018/1/6 19:49:00"
			Utils.appendShipInfo(0, {
				ro: '左满舵'
			});
		}
...
//			Utils.addDescMsg(25, "<b>1949时许</b> 此时，2轮值班水手提醒三副 “1”轮CPA约0.1海里，三副随即要求水手使用手操舵操右舵但未给出具体舵令，舵角操至右舵20度时水手报告三副。");
	...
	{"name":"碰撞过程（1950-1952）","timeStart":0,"index":[],"timeLength":Utils.timeLength("10:51", "11:04"),"time":["2018/1/6 19:50:00","2018/1/6 19:50:13"],select:true,event: function(timePoint, timeEvent){
		//time
		var startTime = timeEvent.time[0];
		var endTime = timeEvent.time[1];
		var interval = (Math.floor(new Date(endTime).getTime()/1000) - Math.floor(new Date(startTime).getTime()/1000))/timeEvent.timeLength; //单位秒
	...
	{"name":"燃烧起火漏油（1950-1952）","timeStart":0,"index":[],"timeLength":Utils.timeLength("11:05", "13:50"),"time":["2018/1/6 19:50:13","2018/1/6 19:52:00"],select:true,event: function(timePoint, timeEvent){
		//time
		var startTime = timeEvent.time[0];
		var endTime = timeEvent.time[1];
		var interval = (Math.floor(new Date(endTime).getTime()/1000) - Math.floor(new Date(startTime).getTime()/1000))/timeEvent.timeLength; //单位秒
	...
	{"name":"2号轮弃船脱离（1952分后）","timeStart":0,"index":[],"timeLength":Utils.timeLength("13:51", "14:30"),"time":["2018/1/6 19:52:00","2018/1/6 19:52:00"],select:true,event: function(timePoint, timeEvent){
		Config.timeSelect = timeEvent.timeStart;
		ArGis.initAccident();
		Utils.animateClear();
		//map
	...
	{"name":"救援、灭火（1.7-1.12）","timeStart":0,"index":[],"timeLength":Utils.timeLength("14:31", "15:04"),"time":["2018/1/7 00:00:00","2018/1/12 23:59:59"],select:true,event: function(timePoint, timeEvent){
		Config.timeSelect = timeEvent.timeStart;
		ArGis.initAccident();
		Utils.animateClear();
		//map
		...
		if(timePoint == timeEvent.timeStart + 1){ 
			...
			Utils.addDescMsg(30, "<b>1月7日1053时</b> “东海救101”轮协助“2号船”轮船员返回本船对船上设备进行检查，并将船控制住。“东海救101”轮和“2号船”轮登轮船员对艏部进行灭火作业。");
		}else if(timePoint == timeEvent.timeStart + 20){ 
			Utils.addDescMsg(31, "<b>1月7日1628时</b> 所有船员全部返船，对船上设备进行检修，由“东海救118”轮全程伴航，驶往舟山靠泊接受海事调查。");
		}
	}},
	{"name":"碰撞后沉没前1轮情况（1.7-1.12）","timeStart":0,"index":[],"timeLength":Utils.timeLength("15:05", "15:18"),"time":["2018/1/7 00:00:00","2018/1/12 23:59:59"],select:true,event: function(timePoint, timeEvent){
		Config.timeSelect = timeEvent.timeStart;
		ArGis.initAccident();
		Utils.animateClear();
		//map
		...
		//message
		if(timePoint == timeEvent.timeStart + 1){ 
			Utils.addDescMsg(32, "<b>1月7日</b> “1号船”轮艏部右舷侧持续剧烈燃烧，船体右倾。");
	...
	{"name":"打捞局登船救援（1.13）","timeStart":0,"index":[],"timeLength":Utils.timeLength("15:19", "15:48"),"time":["2018/1/13 00:00:00","2018/1/13 23:59:59"],select:true,event: function(timePoint, timeEvent){
		Config.timeSelect = timeEvent.timeStart;
		ArGis.initAccident();
		Utils.animateClear();
		//map
	...
	{"name":"沉没（1.14）","timeStart":0,"index":[],"timeLength":Utils.timeLength("15:49", "16:19"),"time":["2018/1/14 00:00:00","2018/1/14 23:59:59"],select:true,event: function(timePoint, timeEvent){
		Config.timeSelect = timeEvent.timeStart;
		ArGis.initAccident();
		Utils.animateClear();
		//map
