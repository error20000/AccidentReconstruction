    ...
    var measureTools = declare(_WidgetBase, {
        ...
        _calDistance: function (point1, point2) {           
            ...
            if (this.defaults.map.spatialReference.isWebMercator() || this.defaults.map.spatialReference.wkid == "4326") {//在web麦卡托投影和WGS84坐标系下的计算方法
                if (this.defaults.map.spatialReference.wkid == "102113") {
                    var geoLine = esri.geometry.webMercatorToGeographic(line);
                    var _distance = esri.geometry.geodesicLengths([geoLine], esri.Units.METERS);
                    return _distance[0];
                }
        ...
        _calArea: function (polygon) {
            ...
            if (spatialReference.isWebMercator() || spatialReference.wkid == "4326") {
                if (this.defaults.map.spatialReference.wkid == "102113") {
                    ...
                    var _distance = esri.geometry.geodesicAreas([geo], esri.Units.SQUARE_METERS);
                    return _distance[0];
                }
        ...
        _startMeasureEBL:function() {
            this._clearMapMouseClickEvent();
            this._measureLayer.clear();
            this.toolbar.deactivate();
            this.toolbar.activate(Draw.CIRCLE);
        }
